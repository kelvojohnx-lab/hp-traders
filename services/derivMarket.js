...
export function startDerivStream(appId = process.env.DERIV_APP_ID || "33zfvRF3DsU2CWrMGx7th") {
  try {
    ...
    ws.on("open", () => {
      console.log("✅ Deriv WS connected — App ID:", appId);
    ...
    ws.on("error", (err) => {
      console.log("⚠️ Deriv WS error:", err.message);
    });
...
export function fetchDerivAccount(token) {
  return new Promise((resolve, reject) => {
    ...
    const timeout = setTimeout(() => {
      conn.close();
      reject(new Error("Deriv account fetch timed out"));
    }, 12000);

    conn.on("open", () => {
      conn.send(JSON.stringify({ authorize: token }));
    });
